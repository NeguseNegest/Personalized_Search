...
def _profile_blend(num_clicks: int) -> tuple[float, float]:
    """
    Explicit profile dominates at cold start.
    Click profile dominates as clicks accumulate.
...
def _compute_bm25_features(hits: list[dict], normalize_bm25: bool) -> dict[str, float]:
    """
    Return a dict: doc_id -> bm25 feature value.

    If normalize_bm25 is True, use min-max normalization over the retrieved hits.
    If False, use the raw BM25 score.
    """
    raw_scores = {hit["_id"]: float(hit.get("_score") or 0.0) for hit in hits}

    if not normalize_bm25 or not raw_scores:
        return raw_scores
    ...
    if math.isclose(max_score, min_score):
        # All hits have the same BM25 score.
...
def _format_baseline_hit(hit: dict, bm25_feature: float | None = None) -> dict:
    src = hit["_source"]
    ...
    return {
        ...
        "bm25_feature": round(float(bm25_feature if bm25_feature is not None else es_score), 3),
        "final_score": round(float(bm25_feature if bm25_feature is not None else es_score), 3),
        "genre_bonus": 0.0,
        "author_bonus": 0.0,
        "book_bonus": 0.0,
        "click_bonus": 0.0,
...
def _rerank_hits(hits: list[dict], profile: dict, query: str, normalize_bm25: bool) -> list[dict]:
    ...
    bm25_features = _compute_bm25_features(hits, normalize_bm25=normalize_bm25)
    ...
    for hit in hits:
        ...
        raw_bm25 = float(hit["_score"] or 0.0)
        bm25_feature = float(bm25_features.get(doc_id, raw_bm25))
        ...
        reranked.append({
            ...
            "genres": book_genres,
            "summary": _truncate_summary(src.get("summary", "")),
            "es_score": round(raw_bm25, 3),              # original Elasticsearch BM25
            "bm25_feature": round(bm25_feature, 3),      # normalized or raw, depending on toggle
...
def search_books(
    ...
    size: int = 10,
    user_id: str = "",
    normalize_bm25: bool = NORMALIZE_BM25,
) -> list[dict]:
    ...
    if not personalized:
        bm25_features = _compute_bm25_features(hits, normalize_bm25=normalize_bm25)
        return [
            _format_baseline_hit(hit, bm25_feature=bm25_features.get(hit["_id"]))
            for hit in hits[:size]
        ]

    profile = get_user_profile(user_id)
    reranked = _rerank_hits(hits, profile, query, normalize_bm25=normalize_bm25)
