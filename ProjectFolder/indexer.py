...
def build_embedding_input(doc: dict) -> str:
    """
    Embed summary only, but keep more text than before.
    """
    summary = (doc.get("summary") or "")[:SUMMARY_CHAR_LIMIT]
...
if __name__ == "__main__":
    path = Path(FILE_PATH)
    if not path.exists():
        raise FileNotFoundError(f"Could not find dataset file: {path.resolve()}")
